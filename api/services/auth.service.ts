...
/**
 * HADRAN FABRICS MALL — Auth service
...
export async function login(username: string, password: string): Promise<AuthResult> {
  ...
  // Brute-force protection: lock a username after repeated failures.
...
export async function changePassword(userId: number, current: string, next: string): Promise<void> {
  ...
  // Kill all other sessions so the old password can't keep working anywhere.
 ...
 *   otherwise   → role matrix (allowed=true), then per-user overrides applied on top.
 */
