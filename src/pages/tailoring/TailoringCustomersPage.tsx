...
import { Crown, Users, UserCheck, CircleDollarSign } from "lucide-react";
...
/**
 * HADRAN FABRICS MALL — tailoring customers
 * Customer records aggregated from tailoring order history (walk-ins
 * included): jobs, lifetime spend, outstanding balance and last visit.
 */
...
export default function TailoringCustomersPage() {
  const [search, setSearch] = useState("");
  const customersQuery = trpc.tailoring.customers.useQuery({ search: search || undefined }, { retry: 1 });
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <PageHeader
        title="Tailoring Customers"
        description="Everyone who has brought in a sewing job — order counts, lifetime spend and outstanding balances."
        actions={<ExportButton filename="tailoring-customers" sheetName="Tailoring Customers" columns={exportColumns} rows={rows} />}
      />

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard icon={Users} label="Customers" value={String(totals.count)} tone="navy" hint="Distinct tailoring customers" />
      ...
      <div className="mt-4">
        <DataTable
          ...
          emptyTitle="No tailoring customers yet"
