...
import { Banknote, CircleDollarSign, Scale } from "lucide-react";
import { PAYMENT_METHODS, PAYMENT_METHOD_LABELS, type LaundryOrderStatus, type PaymentMethod } from "@contracts/constants";
import { LAUNDRY_STATUS_LABELS } from "@contracts/labels";
import { laundryStatusTone } from "@/lib/laundry";
import { formatCurrency, formatDate, formatDateTime } from "@/lib/format";
import { cn } from "@/lib/utils";

/**
 * HADRAN FABRICS MALL — laundry payments
 * Every deposit and balance collection for laundry orders, filterable and
 * exportable. Each row here has a matching entry in the money ledger.
 */
...
interface PaymentRow {
  ...
  orderStatus: LaundryOrderStatus | null;
  amount: string;
...
export default function LaundryPaymentsPage() {
  ...
  const list = trpc.laundry.payments.useQuery(queryInput, { retry: 1 });
  const exportQuery = trpc.laundry.exportPayments.useQuery(
  ...
  const columns: Column<PaymentRow>[] = [
    { header: "When", render: (r) => <span className="whitespace-nowrap text-xs">{formatDateTime(r.createdAt)}</span> },
    {
      ...
      render: (r) =>
        r.orderNo ? (
          <Link to={`/laundry/orders/${r.orderId}`} className="font-mono text-xs font-semibold text-navy-800 underline-offset-2 hover:text-gold-700 hover:underline">
            {r.orderNo}
          </Link>
        ) : (
          "—"
    ...
    { header: "Order Status", render: (r) => (r.orderStatus ? <StatusBadge label={LAUNDRY_STATUS_LABELS[r.orderStatus]} tone={laundryStatusTone(r.orderStatus)} /> : "—") },
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <PageHeader
        ...
        actions={
          <>
            <ExportButton filename="laundry-payments" sheetName="Laundry Payments" columns={exportColumns} rows={async () => ((await exportQuery.refetch()).data ?? []) as PaymentRow[]} />
      ...
      <div className="mt-6">
        <DataTable
          ...
          keyFn={(r) => r.id}
          emptyTitle="No laundry payments found"
      ...
      {printRows && (
        <PrintPortal>
          <div className="p-6">
            <h1 style={{ fontSize: 18, fontWeight: 700 }}>Hadran Fabrics Mall — Laundry Payments</h1>
